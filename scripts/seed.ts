...
const USER_ID = "69efa343a51a3a482a80c921";
...
async function seed() {
  ...
  try {
    ...
    if (!board) {
      ...
      const  {  initBoard }  = await import('../lib/initUserBoard');
      board = await initBoard(USER_ID);
    ...
    // Get all columns
    const columns = await Column.find({ board: board._id }).sort({
      order: 1,
    });
